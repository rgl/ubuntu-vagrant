...
ubuntu-${VERSION}-amd64-libvirt.box: preseed.txt provision.sh ubuntu.json Vagrantfile.template
	...
	PACKER_KEY_INTERVAL=10ms CHECKPOINT_DISABLE=1 packer build -only=ubuntu-${VERSION}-amd64-libvirt -on-error=abort ubuntu.json
...
ubuntu-${VERSION}-amd64-virtualbox.box: preseed.txt provision.sh ubuntu.json Vagrantfile.template
	...
	CHECKPOINT_DISABLE=1 packer build -only=ubuntu-${VERSION}-amd64-virtualbox -on-error=abort ubuntu.json
	@echo BOX successfully built!
	@echo to add to local vagrant install do:
